...
async function fillingFieldsOfCover() {
  ...
  (async () => {
    for (let i = 0; i < classes.length; i++) {
      ...
      // await page.goto("https://devgl96.github.io/carneEscolarCrescer-v1/");

      await page.waitForSelector("#coverCarneWithName");
      await page.click("#coverCarneWithName");

      await page.waitForSelector("#formInputCover");
      await page.waitForSelector("#meuFormInputCover");
      await page.waitForSelector("#nomeAlunoCover");

      for (let j = 0; j < studentList[classes[i]].length; j++) {
        let nameInput = await page.$("#nomeAlunoCover");

        // Input: Nome Completo
        await nameInput.click();
        await nameInput.type(studentList[classes[i]][j]["name"]);
  ...
  (async () => {
    for (let i = 0; i < classes.length; i++) {
      for (let j = 0; j < studentList[classes[i]].length; j++) {
        ...
        await page.type(
          "input[placeholder='Digite o Valor da Mensalidade']",
          studentList[classes[i]][j]["value"]
        );
